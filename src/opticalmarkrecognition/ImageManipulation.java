    ...
    public void searchMarks(String projName) {
        ...
        for(int i = 0; i < realNummarks; i++) {
            ...
            if(mark.isMark(x, y)) {
                ...
                DBHelper dbh = new DBHelper();
                String qry1 = "SELECT "+field.getName()+" FROM "+projName+" WHERE "+first+" = '"+second+"'";
                String val = dbh.processQRY(qry1, field.getName());
                if(val!=null){
                    String qry="UPDATE "+projName+" SET "+field.getName()+" = '"+field.getValue(i)+"' WHERE "+first+" = '"+second+"'";
                    dbh.testing(qry);
                }else{
                    String qry="UPDATE "+projName+" SET "+field.getName()+" = null WHERE "+first+" = '"+second+"'";
                    dbh.testing(qry);
                }
